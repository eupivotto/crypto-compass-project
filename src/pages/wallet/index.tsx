  ...
  const handleGetTransfer = async () => {
    if (!account) {
      dispatch(
        setSnackbar({
          open: true,
          message: "Carteira não conectada.",
          severity: "error",
        })
      );
      return;
    ...
    try {
      ...
      dispatch(
        setSnackbar({
          open: true,
          message: `Valor enviado com sucesso! Transação: ${transactionHash}`,
          severity: "success",
        })
      );
      ...
      dispatch(
        setSnackbar({
          open: true,
          message: "Erro ao enviar o valor.",
          severity: "error",
        })
      );
      console.error(error);
    ...
    <div className=" flex flex-col container-main ">
      ...
      <div className="flex items-center justify-center w-100">
        <div className="send-container w-100 ">
          ...
          <Input
            ...
            onChange={(ev: { target: { value: any } }) => {
              ...
              if (
                String(value) === "" ||
                String(value) === "." ||
                (!isNaN(Number(value)) && String(value).split(".").length === 2)
              ) {
                SetQuantity(value);
              } else {
          ...
          />
          <Button className="button-send" onClick={handleGetTransfer}>
            Enviar
          </Button>
        </div>
      </div>
