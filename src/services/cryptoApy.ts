...
export const fetchChartData = async (coinId: string) => {
  try {
    ...
    const chartData = {
      labels: sampledPrices.map((price: [number, number]) =>
    ...
    if (axios.isAxiosError(error) && error.response) {
      ...
      console.error(
        `Erro ao buscar dados do gráfico: ${error.response.status} ${error.response.data.message}`
      );
    } else {
